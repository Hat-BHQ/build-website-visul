...
assert(source.includes('/hqa/dashboard/analysis?${'), 'Dashboard must load one aggregate analysis payload from the API');
assert(source.includes('function syncDashboardFiltersFromAllListings()'), 'Dashboard must sync the existing All Listings applied filters');
assert(source.includes('state.hqa.allListings.appliedFilters'), 'Dashboard filter scope must originate from All Listings');
assert(source.includes("appendDashboardArrayParams(params, 'condition', filters.conditions)"), 'Dashboard must reuse condition filters');
assert(source.includes("appendDashboardArrayParams(params, 'status', filters.statuses)"), 'Dashboard must reuse status filters');
assert(source.includes("appendDashboardArrayParams(params, 'category_name', filters.categoryNames)"), 'Dashboard must reuse category filters');
assert(source.includes("appendDashboardArrayParams(params, 'buying_option', filters.buyingOptions)"), 'Dashboard must reuse buying option filters');
assert(!source.includes('id="dashboard-filters"'), 'Dashboard must not render a second global filter form');
assert(!source.includes('id="dashboard-seller-stats-filters"'), 'Dashboard must not render a separate seller KPI date filter');
assert(source.includes('const DASHBOARD_FIELD_MAP = {'), 'Dashboard must centralize listing field mapping');
assert(source.includes("groupBy: 'model'"), 'Dashboard must group by Model by default');
assert(source.includes("granularity: 'month'"), 'Dashboard must aggregate by month by default');
assert(source.includes('id="dashboard-group-by"'), 'Dashboard must allow switching aggregation between Model, Brand and Category');
assert(source.includes('id="dashboard-granularity"'), 'Dashboard must allow switching period grouping between month and week');
assert(source.includes("supportedCurrencies: ['USD', 'VND']"), 'Dashboard currency formatter must explicitly support USD and VND');
assert(source.includes('priceDropWarningPct: 20'), 'Price drop warning threshold must be configurable at 20%');
assert(source.includes('priceDropCriticalPct: 30'), 'Price drop critical threshold must be configurable at 30%');
assert(source.includes('outOfStockWarningPoints: 30'), 'Out-of-stock warning threshold must be configurable at 30 percentage points');
assert(source.includes('outOfStockCriticalPoints: 50'), 'Out-of-stock critical threshold must be configurable at 50 percentage points');
assert(source.includes('new_seller_count'), 'Dashboard must expose new seller analytics');
assert(source.includes('renderDashboardRangeSvg'), 'Dashboard must render the P25-P75 price range SVG');
assert(source.includes('renderDashboardSparkline'), 'Dashboard must render a six-period price sparkline');
assert(source.includes('Number(current.min_price) * 0.97'), 'Fast-sale recommendation must use min price x 0.97');
assert(source.includes("recommended = 'Lợi nhuận cao'"), 'Recommendation logic must support the high-profit tier');
assert(source.includes("recommended = 'Bán nhanh'"), 'Recommendation logic must support the fast-sale tier');
assert(source.includes('renderDashboardTopSellers'), 'Dashboard must render Top 10 sellers for selected Model + period');
assert(source.includes('data-dashboard-export="group_period"'), 'Dashboard must export group-period CSV');
assert(source.includes('data-dashboard-export="alerts"'), 'Dashboard must export alerts CSV');
assert(source.includes('data-dashboard-export="top_sellers"'), 'Dashboard must export Top 10 sellers CSV');
assert(source.includes('\\ufeff'), 'Dashboard CSV export must include UTF-8 BOM');
assert(source.includes('new Chart(priceCanvas'), 'Dashboard must render Chart.js price trend');
assert(source.includes('new Chart(sellerCanvas'), 'Dashboard must render Chart.js seller trend');
assert(styles.includes('#hqa-dashboard-view.panel'), 'Dashboard must have the mockup-aligned light analytics canvas');
assert(styles.includes('.dashboard-kpi-card'), 'Dashboard KPI card styles must exist');
assert(styles.includes('.dashboard-alert-list'), 'Dashboard alert radar styles must exist');
assert(styles.includes('.dashboard-chart-grid'), 'Dashboard chart layout styles must exist');
assert(styles.includes('.dashboard-top-seller-row'), 'Top seller row styles must exist');
